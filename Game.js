class Game {
  ...
  getState(){
    var gameStateRF=database.ref('gameState')
    gameStateRF.on('value',function(data){
      gameState=data.val()
    })
  }
  update(state){
  ...
  start() {
    ...
    gasGroup=new Group()
    coinGroup=new Group()
    this.addSprites(gasGroup,5,gasImg,0.02);
    this.addSprites(coinGroup,5,coinImg,0.07);
...
addSprites(spriteGroup,spriteQuantity,spriteImage,spriteScale){
  for(var i=0;i<spriteQuantity;i++){
    var x,y
